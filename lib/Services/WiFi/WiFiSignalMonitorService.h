...
public:
    ...
    static bool start(const WiFiScannerService::Network &target);
    static void stop();
    static void update();
    ...
    static std::int32_t sample(std::uint8_t index);
    static std::uint32_t successfulScans();
    static std::uint32_t missedScans();
    static std::int16_t lastScanCode();

private:
    static constexpr std::uint32_t SampleIntervalMs = 1500;
    static constexpr std::uint32_t RetryDelayMs = 1000;

    static bool requestSample();
    static void consumeResults(std::int16_t resultCount);
    static void addSample(std::int32_t rssi);
    static void recalculateStatistics();
    static bool bssidMatches(const String &left, const String &right);
